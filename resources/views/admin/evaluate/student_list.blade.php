        ...
        <div class="card">
            ...
            <div class="card-body table-responsive p-0">								
                <table class="table table-hover text-nowrap">
                    ...
                    <tbody>
                        <?php $i =1;?>
                        @foreach ($assignedStudents as $studenlist)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{$studenlist->student_name ?? 'Demo'}}</td>
                            {{-- <td> {{$studenlist->email}}</td> --}}
                            <td>
                                <a href="{{ route('checked.list', $studenlist->student_id) }}" class="btn btn-primary">View Checked Answers</a>
                            </td>
                            {{-- <td> 
                                @foreach($studenlist->states as $state)
