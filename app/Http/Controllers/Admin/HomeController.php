...
class HomeController extends Controller
{
    ...
    public function index(){
        ...
        $activePlansCount = DB::table('user_plans')
           ->where('status', 'active')
           ->count();

        return view('admin.dashboard',compact('checkedStudents','doubtsCount','activePlansCount'));
    }
    ...
    public function doubtStudents()
    {
        $studentsWithDoubts = DB::table('doubts')
            ->join('users', 'users.id', '=', 'doubts.user_id')
            ->select('users.id', 'users.name', 'users.email')
            ->distinct()
            ->get();

        return view('admin.doubtstudent', compact('studentsWithDoubts'));
    }

    public function studentDoubts($studentId)
    {
       $doubts = DB::table('doubts')
        ...
        ->orderByRaw("CASE WHEN doubts.status = 'pending' THEN 0 ELSE 1 END")
    ...
    public function doubtDlt($id)
    {
        ...
        return redirect()->back()->with('success', 'Doubt deleted successfully!');
    }
